        ...
        <v-list-item three-line>
          ...
          <v-card-actions>
            <v-dialog v-model="user.openDialog" max-width="600px">
              ...
              <v-card>
                <v-card-title>
                  <span class="text-h5">Change User</span>
                </v-card-title>
                <v-card-text>
                  <v-container>
                    <v-row>
...
export default {
  ...
  computed: {
    ...
    filteredUsers() {
      return this.getUsers.filter((user) => user.status === 1);
    },
  },
  methods: {
    async editUser(userId) {
      ...
      try {
        const response = await axios.get(`getOneUser/api`);
        this.getOneUser = response.data.result.users;

        // Podesi izabrane grupe na osnovu korisnika koji se trenutno menja
        this.selectedGroup = this.getOneUser.groups.map((group) => group.name);
      } catch (error) {
    ...
    async deleteUser(user) {
      const index = this.getUsers.findIndex((u) => u.id === user.id);
      if (index !== -1) {
        this.getUsers.splice(index, 1);
      }

      try {
        const response = await axios.post("deleteUser/Api", {
          userId: user.id,
          deactivateUser: true,
        });
        this.snackbar.message = "You have successfully delete user";
    ...
    async saveChanges(user, selectedGroup) {
      try {
        const data = {
          userId: user.id,
          name: user.name,
          groupIds: selectedGroup.map((groupName) => {
        ...
        console.log(data);
        await axios.post("editUser/api", data);
